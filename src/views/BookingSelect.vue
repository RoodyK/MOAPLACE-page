        ...
        <main>
                 <div class="info">
                    ...
                    <div class="showInfo">
                        ...
                        <p>기간 : {{list.startDate.substr(5,2)}}월 {{list.startDate.substr(8,2)}}일 ~ {{list.endDate.substr(5,2)}}월 {{list.endDate.substr(8,2)}}일</p>
                        <p>장소 : {{list.hall}}</p>
                    </div>
                 </div>
                 <div class="date">
                    ...
                    <div class="selectDate" @scroll="getScroll($event.target)">
                        <button  v-for="(m,index) in currentMonth" :key="index" ref='sc' :class="{sat:m.day=='토',sun:m.day=='일',firstDay:m.date.getDate()=='1'}" class="static" @click="viewTime($event.currentTarget)" :value="m.date.getFullYear()+'-'+('0'+(m.date.getMonth()+1)).slice(-2)+'-'+('0'+m.date.getDate()).slice(-2)">
                 ...
                 <div class="time">
                    ...
                    <div class="timeBase" v-if="timeTable.length>0">
                        <div v-for="(m,index) in timeTable" :key="index" :class='{"timeTable":true}' class='static' ref="tar" @click="selectTime($event.currentTarget.textContent)">
                            <p>{{m.count}}</p>
                            <p>{{m.startTime}}</p>
                            <p>{{m.seat}}</p>
                        </div>
        ...
        viewDate(){
            ...
            for(let i=this.list.today;i<=this.list.today+this.getTerm();i++){ //오늘 날짜에서 마지막 공연날까지의 일수를 더한만큼 반복 (오늘 날짜부터 i 시작)
                this.currentMonth.push(
                    {
                        day:this.day[new Date(this.list.year,this.list.month-1,i).getDay()], //요일 구하기
        ...
        viewTime(e){
            ...
            for(let i=0;i<this.list.time.length;i++){
                if(this.list.time[i].showDate==e.value){
                    this.onDate=e.value.substr(2,2)+"."+e.value.substr(5,2)+"."+e.value.substr(8,2)+"("+e.textContent.substr(0,1)+")";
                    this.timeTable.push(
                        {
                            count:(ie++)+'회차',
